...
| File | Covers |
| --- | --- |
| `tests/unit/core/test_game_rules.gd` | Humans pay the wallet they were given, count down, the last human wins exactly once, nothing after the win, negative values, an empty level is born won, a missing wallet is reported and replaced (12 tests) |
| `tests/unit/core/test_wallet.gd` | Earn adds and emits, fines subtract, never below $0, a fine against $0 is silent, `reset_to` puts money back (silent if unchanged), nothing-or-less is ignored, starting money (13 tests) |
| `tests/unit/core/test_patrol.gd` | Starts left, turns at a wall, turns at a ledge, turns back again, flips every step on a sliver, nonsense start direction (8 tests) |
| `tests/unit/core/test_enemy_contact.gd` | Falling onto the top is a stomp, including the late-reported contact measured in the real game and a fast fall; the side (even at the fall cap), sinking too deep for the speed, rising past it or standing still are touches; the tolerance boundary is exact (9 tests) |
| `tests/unit/core/test_level_config.gd` | Defaults valid, `level_01.tres` loads with the designer's numbers, a free stomp and no fine are allowed, zero/negative rejected |
| `tests/unit/core/test_platformer_motion.gd` | Run speed and input clamping, gravity per step, fall cap reached and held, jump only from the floor, landing cancels downward speed but keeps an upward kick, `configure()`, nonsense tunables clamped (14 tests) |
| `tests/integration/player/test_player.gd` | Falls and lands on a `world`-layer floor, idle stays still, runs on `move_right` at `speed`, jumps on `jump`, no double jump, `feet_y()` sits on the floor, `bounce()` kicks it up, non-default tunables reach the motion even after `_ready` |
...
| `tests/integration/strawberry/test_strawberry.gd` | Layers pinned (enemy, masks world, hitbox masks player), walks left, turns at a wall and keeps going, turns at a ledge instead of falling, the blob landing on top stomps it (bounce measured at the stomp), the blob walking into its side is caught |
| `tests/integration/ui/test_pop_text.gd` | Shows its text, floats up, fades, frees itself |
...
| `tests/integration/main/test_main.gd` | Screen flow title → level → results → retry/title, Main's level scene is the one played, money carries to the next level, the title screen starts a new job, `blob_caught` rebuilds the level with the same wallet, a real catch (eat, then walk into a strawberry) rebuilds the level with the attempt's earnings forfeited and every human back, the results screen shows the wallet not the bus number, un-pause on every swap, deferred `game_over` |
| `tests/integration/level/test_level_flow.gd` | A whole level: the designer's human count (pinned once), HUD labels read through the level, every human paid the config's value, eating pays and emits exactly once, eating everyone wins exactly once and disables pausing, not won with one left, an empty level ends at once, a stray node under `Humans` is ignored, quitting from the pause menu ends the level as `LOST` with the money so far, a level given a wallet adds to it and stops relaying it once out of the tree, strawberries start on ground and are still patrolling it 1.5 s later, layers pinned, a stomp pays the config's `stomp_value` and pops it, a stomp after the win pays nothing, being caught forfeits the attempt's earnings then takes the fine (never below $0), pops the real loss, freezes the world and emits `blob_caught` once — even with two strawberries — after the caught pause, nothing pays or finishes once caught, a stray node under `Strawberries` is ignored, a level without a `Strawberries` node still plays, eating pops `+$` at the human's spot, pause; the world: player drops in and lands, every `World` child is solid, walls hold, every piece of ground is within one jump of a lower one, every human stands on something |

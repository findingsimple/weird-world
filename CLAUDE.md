...
A Godot **4.7.2** + **GDScript** game-project template with a small example game
(**Weird World**, a top-down coin collector). New games are created from it via
"Use this template" + `scripts/new_game.py`. Full docs live in `docs/`; start with
...
4. **Logic lives in `RefCounted` classes** (`GameRules`, `CoinSpawner`) with no scene
...
9. **Text-only assets in the template** (hand-written SVG). No binaries, no Git LFS
   (template repos can't use it). Real games add assets per `ASSETS.md`.
...
- `Coin` (`Area2D`, layer 2, mask 1) emits `collected` when a `Player` (`CharacterBody2D`,
  layer 1) overlaps, then frees itself. `Level` turns that into `GameRules.add_score`.
...
- Input actions (`move_left/right/up/down`, `pause`) are in `project.godot` → `[input]`.
  Physics layers: 1 `player`, 2 `pickups`.
...
- **A new input action:** add it to `project.godot` `[input]` (copy an existing block; use
  `physical_keycode`), then read it with `Input.is_action_pressed` / `Input.get_vector`.

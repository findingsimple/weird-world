...
##
## Owns a [GameRules] (the logic), tells it how many humans the scene placed, and forwards
## its signals to [GameEventsBus] so the HUD and [Main] can react. Pausing — and leaving the
## level from the pause menu — is handled with the child [PauseMenu]. The floor, the
## platforms, the walls, the humans and the blob's start are all placed by hand in
## level.tscn — that is level design.
...
func _ready() -> void:
	...
	_rules = GameRules.new(humans.size())
	_rules.money_changed.connect(_on_rules_money_changed)
	...
	GameEvents.money_changed.emit(_rules.money)
...
func _on_pause_menu_quit_pressed() -> void:
	# Leaving is not winning: the level ends as LOST with whatever was earned so far.
	_pause_menu.enabled = false
	GameEvents.game_over.emit(GameRules.Outcome.LOST, _rules.money)


func _on_rules_money_changed(money: int) -> void:
...
func _on_rules_finished(outcome: GameRules.Outcome) -> void:
	_pause_menu.enabled = false
	GameEvents.game_over.emit(outcome, _rules.money)
